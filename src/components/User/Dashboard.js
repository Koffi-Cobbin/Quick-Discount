...
function DiscCard({ d, isSaved, onSave, idx }) {
  ...
  return (
    <a
      className="card"
      href={`/discounts/${d.id}`}
      style={{ animationDelay: `${Math.min(idx * 0.055, 0.28)}s` }}
    >
      <div className="cimg">
        {d.img ? (
        ...
        {exp && (
          <div className={`cexp ${exp.expired ? "ex" : "ok"}`}>{exp.label}</div>
        )}
        <button
          className={`cheart${isSaved ? " sv" : ""}`}
          onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
            onSave(d.id);
      ...
      <div className="cbody">
        ...
        <div className="cmeta">
          {d.org && (
            <div className="mrow">
              <span>👤</span>
              <span className="mtxt">{d.org}</span>
            </div>
          )}
          {d.loc && (
            <div className="mrow">
              <span>📍</span>
              <span className="mtxt">{d.loc}</span>
            </div>
          )}
          {d.start && (
            <div className="mrow">
              <span>📅</span>
              <span className="mtxt">{fmtDate(d.start)}</span>
            </div>
          )}
        </div>
        <div className="cfoot">
          ...
          {d.rating > 0 && (
            <span className="stars">★ {d.rating.toFixed(1)}</span>
          )}
        </div>
      </div>
    </a>
